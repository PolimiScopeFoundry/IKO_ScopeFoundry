...
class IKO_Device(object):
    ...
    '''
        ...
        def get_velocity(self):
            ...
            # else:
                return sp.GetVelocity(self.hc, 0, sp.SYNCHRONOUS, True)

        def set_velocity(self, desired_velocity):
            # if hasattr(self, 'hc_sim'):
            #     sp.SetVelocity(self.hc_sim, 0, desired_velocity, sp.SYNCHRONOUS, True)
            # else:
                sp.SetVelocity(self.hc, 0, desired_velocity, sp.SYNCHRONOUS, True)
        ...
        def get_acceleration(self):
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetAcceleration(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetAcceleration(self.hc, 0, sp.SYNCHRONOUS, True)
        
        def set_acceleration(self, desired_acceleration):
            # if hasattr(self, 'hc_sim'):
            #     sp.SetAcceleration(self.hc_sim, 0, desired_acceleration, sp.SYNCHRONOUS, True)  
            # else:
                sp.SetAcceleration(self.hc, 0, desired_acceleration, sp.SYNCHRONOUS, True)



        def get_deceleration(self):
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetDeceleration(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetDeceleration(self.hc, 0, sp.SYNCHRONOUS, True)
        
        def set_deceleration(self, desired_acceleration):
            # if hasattr(self, 'hc_sim'):
            #     sp.SetDeceleration(self.hc_sim, 0, desired_acceleration, sp.SYNCHRONOUS, True)  
            # else:
                sp.SetDeceleration(self.hc, 0, desired_acceleration, sp.SYNCHRONOUS, True)



        def get_jerk(self):
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetJerk(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetJerk(self.hc, 0, sp.SYNCHRONOUS, True)
            
        def set_jerk(self, desired_jerk):
            # if hasattr(self, 'hc_sim'):
            #     sp.SetJerk(self.hc_sim, 0, desired_jerk, sp.SYNCHRONOUS, True) 
            # else:
                sp.SetJerk(self.hc, 0, desired_jerk, sp.SYNCHRONOUS, True)
        ...
        def get_fposition(self):#feedback position: a measured position of the motor transferred to user units.
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetFPosition(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetFPosition(self.hc, 0, sp.SYNCHRONOUS, True)
            
        def set_fposition(self, desired_pos): #motor does not move FPOS
            # if hasattr(self, 'hc_sim'):
            #     sp.SetFPosition(self.hc_sim, 0, desired_pos, sp.SYNCHRONOUS, True) 
            # else:
                sp.SetFPosition(self.hc, 0, desired_pos, sp.SYNCHRONOUS, True)

        def get_fvelocity(self):#feedback velocity FPOS
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetFVelocity(self.hc_sim, 0, sp.SYNCHRONOUS, True)    
            # else:
                return sp.GetFVelocity(self.hc, 0, sp.SYNCHRONOUS, True)
        ...
        def get_rposition(self):#reference position RPOS
            # if hasattr(self, 'hc_sim'): 
            #     return sp.GetRPosition(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetRPosition(self.hc, 0, sp.SYNCHRONOUS, True)
        ...
        def set_rposition(self): #motor does not move RPOS:
            ...
            # else:       
                sp.SetRPosition(self.hc, 0, 0, sp.SYNCHRONOUS, True)
        
        def get_rvelocity(self):#reference velocity
            # if hasattr(self, 'hc_sim'):
            #     return sp.GetRVelocity(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                return sp.GetRVelocity(self.hc, 0, sp.SYNCHRONOUS, True)
            

        def move_absolute(self, desired_pos): #move to a given position
            # if hasattr(self, 'hc_sim'):
            #     sp.ToPoint(self.hc_sim, 0,sp.Axis.ACSC_AXIS_0, desired_pos, sp.SYNCHRONOUS, True)
            # else:
                sp.ToPoint(self.hc,flags= 0, axis = sp.Axis.ACSC_AXIS_0, point = desired_pos, wait = sp.SYNCHRONOUS ,failure_check=True)
        ...
        def move_relative(self, desired_step):
            # if hasattr(self, 'hc_sim'):
            #     sp.ToPoint(self.hc_sim, sp.MotionFlags.ACSC_AMF_RELATIVE,sp.Axis.ACSC_AXIS_0, desired_step, sp.SYNCHRONOUS, True)
            # else:
                sp.ToPoint(self.hc, sp.MotionFlags.ACSC_AMF_RELATIVE,sp.Axis.ACSC_AXIS_0, desired_step, sp.SYNCHRONOUS, True)
        ...
        def move_sequence(self, step, step_num, start_pos):
            ...
            # else:
                #the created motion starts only after the first point is specified. 
                sp.MultiPoint(self.hc, 0, sp.Axis.ACSC_AXIS_0, 1,True)
                for i in range(step_num):
                    sp.AddPoint(self.hc, sp.Axis.ACSC_AXIS_0, start_pos+i*step)
                sp.EndSequence(self.hc, sp.Axis.ACSC_AXIS_0)
        ...
        def wait_on_target(self): 
            ...
            # else:
                sp.WaitMotionEnd(self.hc, 0, 500, True) # timeout in ms
        ...
        def deactivate(self): #disable motor
            # if hasattr(self, 'hc_sim'):
            #     sp.Disable(self.hc_sim, 0, sp.SYNCHRONOUS, True)
            # else:
                sp.Disable(self.hc, 0, sp.SYNCHRONOUS, True)
